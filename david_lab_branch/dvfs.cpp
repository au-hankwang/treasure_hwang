...
void dvfstype::PrintSystem(FILE* fptr, bool temp_only)
   ...
   for (i = 0; i < topology.size(); i++)
      ...
      if (fptr == NULL) printf("%5.1fmV %4.1fC %6.1f/%6.1fMhz %5.1f%% %5.1f%%", voltage[i] * 1000.0, temp[i], frequency[i], frequency2[i], t.bisthitrate * 100.0, t.missionhitrate * 100.0);
      else fprintf(fptr, "%5.1fmV %4.1fC %6.1f/%6.1fMhz %5.1f%% %5.1f%%", voltage[i] * 1000.0, temp[i], frequency[i], frequency2[i], t.bisthitrate * 100.0, t.missionhitrate * 100.0);
      }
   if (fptr == NULL) printf("\n"); else fprintf(fptr, "\n");
   }
...
float dvfstype::MeasureHashRate(int period, bool& too_hot, float& avg_temp, float& max_temp, int experiment) {
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      t.bisthitrate = (float)bist_end[i].data / (bist_iterations * 8 * 238);
      total_f += f;
      bist_total += f * t.bisthitrate;
   ...
   hit_rate = true_total / expected_total;   // return the mission mode hit rate
...
float dvfstype::MeasureHashRate(int period, bool& too_hot, float& avg_temp, float& max_temp, int experiment) {
   ...
   vector<int> partition_count(topology.size(), 0.0);
   int i, end_entries = 0;
   const int bist_iterations = 100;
   bool second_partition_present = false;
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      for (int k = 0; k < 8; k++) {
         ...
         partition_count[i] += PopCount(t.partition[k]);
         second_partition_present = second_partition_present || partition_count[i] != 0;
         }
   ...
   for (i = 0; i < topology.size(); i++) {
      topologytype& t = topology[i];
      t.temperature = t.Temp(batch_end[i * end_entries + 3].data);
      // for long measurement times, E_CLKCOUNT will overflow. To prevent this zero it each interval and accumulate here
      seconds[i] += batch_end[i * end_entries + 0].data / 25.0e6;
      seconds2[i] += batch_end2[i * end_entries + 0].data / 25.0e6;
      float expected = seconds[i] * t.frequency * (238 - partition_count[i]) * (4 / 2.0) * 1.0e6 / ((__int64)1 << 32);
      float expected2 = seconds2[i] * t.frequency2 * partition_count[i] * (4 / 2.0) * 1.0e6 / ((__int64)1 << 32);
      expected_total += expected;
      expected_total += expected2;
      true_total += batch_end[i * end_entries + 1].data;
      true_total += batch_end2[i * end_entries + 1].data;
      t.missionhitrate = batch_end[i * end_entries + 1].data / MAXIMUM(1, expected);
      t.missionhitrate2 = batch_end2[i * end_entries + 1].data / MAXIMUM(1, expected2);
      t.bisthitrate = (float)bist_end[i].data / (bist_iterations * 8 * (238 - partition_count[i]));
      t.bisthitrate2 = (float)bist_end2[i].data / (bist_iterations * 8 * partition_count[i]);
      total_f += t.frequency + t.frequency2;
      bist_total += t.frequency * t.bisthitrate + t.frequency2 * t.bisthitrate2;
...
float dvfstype::MeasureHashRateHybrid(int period, bool& too_hot, float& avg_temp, float& max_temp, int experiment) {
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      total_f += f;
      t.bisthitrate = MAXIMUM(phase2[i].bisthitrate, phase4[i].bisthitrate);
   ...
   for (i = 0; i < boards.size(); i++) {
      topologytype& t = topology[i];
      t.phase2 = 0.0;
      if (phase2[i].bisthitrate > phase4[i].bisthitrate) {
...
void dvfstype::PrintSummary(FILE *fptr, const char* label, float &efficiency)
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      average_v += t.voltage;
      effective_f += t.frequency * t.bisthitrate;
      meffective_f += t.frequency * t.missionhitrate;
      total_f += t.frequency;
      minhit = MINIMUM(minhit, t.bisthitrate);
      maxhit = MAXIMUM(maxhit, t.bisthitrate);
...
void dvfstype::Partition(int experiment)
   ...
   const int bist_delay_ms = 1000.0 * pll_delay * ((2500 - 100) / 5) / 25.0e6;
   const int max_chips = 500;
   const float bist_threshold = 0.5;
...
//                  problem_asics.push_back(index);  // don't duplicate
      ...
      for (i = 0; i < topology.size(); i++)
         ...
         if (k < 0)  // exclude any problem asics from frequency swapping
            ranking.push_back(ranktype(i, t.missionhitrate, t.frequency));
         phase2 += t.phase2;
         total_f += t.frequency;
         }
      std::sort(ranking.begin(), ranking.end());
      ...
      for (i = 0; i < topology.size() / 4; i++)
         ...
         if (i==0)
            printf("Iteration %2d/%d: total=%5.1f%% fast=%5.1f%% slow=%5.1f%% phase2=%5.1f%% AvgTemp=%5.1fC MaxTemp=%5.1fC\n", its, max_its,hitrate*100.0,fast.missionhitrate*100.0, slow.missionhitrate * 100.0, 100.0*phase2/total_f,avg_temp, max_temp);

         if (slow.frequency >= average_f * 0.4 && fast.temperature < avg_temp+15.0)
            {
            float step = fast.missionhitrate - slow.missionhitrate > 0.4 ? 0.05 :
                         fast.missionhitrate - slow.missionhitrate > 0.2 ? 0.03 :
                         fast.missionhitrate - slow.missionhitrate > 0.1 ? 0.02 : 0.01;
   ...
   for (its = 0; its < max_its; its++)
      ...
      for (i = 0; i < topology.size(); i++)
         ...
         if (k < 0)  // exclude any problem asics from frequency swapping
            ranking.push_back(ranktype(i, t.missionhitrate, t.frequency));
         if (k < 0)  // exclude any problem asics from frequency swapping
            ranking.push_back(ranktype(i+topology.size(), t.missionhitrate2, t.frequency2));
         phase2 += t.phase2;
         total_f += t.frequency;
         }
      std::sort(ranking.begin(), ranking.end());
...
bool dvfstype::DVFS(const float THS, int experiment, bool verbose)     // returns true if performance could not be achieved or some error happened
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      for (int k = 0; k < 8; k++) {
         t.partition[k] = 0;
         batch.push_back(batchtype().Write(t.board, t.id, E_ENGINEMASK + k, t.partition[k]));
         }
      batch.push_back(batchtype().Write(t.board, t.id, E_DUTY_CYCLE, 0x80a0));
