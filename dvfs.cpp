...
void dvfstype::PrintSystem(FILE *fptr, bool temp_only)
   ...
   for (i = 0; i < topology.size(); i++)
	 ...
	 if (fptr == NULL) printf(" | "); else fprintf(fptr, " | ");
      ...
      if (fptr == NULL) printf("%5.1fmV %4.1fC %6.1fMhz %5.1f%% %5.1f%%", voltage[i] * 1000.0, temp[i], frequency[i], t.hitrate * 100.0, t.missionhitrate * 100.0);
            else fprintf(fptr, "%5.1fmV %4.1fC %6.1fMhz %5.1f%% %5.1f%%", voltage[i] * 1000.0, temp[i], frequency[i],t.hitrate*100.0, t.missionhitrate * 100.0);
      }
   if (fptr == NULL) printf("\n"); else fprintf(fptr, "\n");
   }
...
float dvfstype::MeasureHashRate(int period, bool& too_hot, float& avg_temp, float& max_temp, int experiment, bool no_reset) {
   ...
   for (i = 0; i < topology.size(); i++) {
      topologytype& t = topology[i];
      if (!no_reset) {
         batch_start.push_back(batchtype().Write(t.board, t.id, E_DUTY_CYCLE, experiment ? 0 : 0x80a0));
         batch_start.push_back(batchtype().Write(t.board, t.id, E_HASHCONFIG, 1 << 16));
         batch_start.push_back(batchtype().Write(t.board, t.id, E_HASHCONFIG, experiment ? 4 : 0));
         batch_start.push_back(batchtype().Write(t.board, t.id, E_BIST, 1));
         batch_start.push_back(batchtype().Write(t.board, t.id, E_CLKCOUNT, 0)); // add some delay so bist finishes and a nonce exhaust completes
         }
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      if (experiment == 2)t.hitrate = t.missionhitrate;
      total_f += f;
...
void dvfstype::PrintSummary(FILE *fptr, const char* label, float &efficiency)
   ...
   float perf = effective_f * 238 * (4 / 2) * 1.0e-6;
...
//   sprintf(buffer,"%sVoltage=%.3fV/%.1fmV %.0fW hitrate=%5.1f%%(%5.1f) Perf=%.1fTH/s Eff=%.2fJ/TH AvgTemp=%.1fC MaxTemp=%.1fC IR=%.1fmV Fan=%.0f%%\n", label, voltage, average_v / topology.size() * 1000.0, power, hitrate * 100.0, (maxhit-minhit)*100.0, perf, MINIMUM(99.99,efficiency), avgtemp, maxtemp, ir * 1000.0,fan_percentage*100.0);
   sprintf(buffer, "%sVoltage=%.3fV/%.1fmV %.0fW Bhit=%5.1f%% Mhit=%5.1f%% Perf=%.1fTH/s Eff=%.2fJ/TH AvgTemp=%.1fC MaxTemp=%.1fC Fan=%.0f%%\n", label, voltage, average_v / topology.size() * 1000.0, power, hitrate * 100.0, 100.0*mhitrate, perf, MINIMUM(99.99, efficiency), avgtemp, maxtemp, fan_percentage * 100.0);
...
void dvfstype::Mine()
   ...
   while (true) {
      ...
      bool too_hot;
      MeasureHashRate(5 * 1000, too_hot, avg_temp, max_temp, 0, true);
...
void dvfstype::PerEngineTweak(int experiment)
   ...
   float phase2_speed[num_engines][238], phase4_speed[num_engines][238];
   ...
   for (b = 0; b < boards.size(); b++) {
      ...
      batch.push_back(batchtype().Write(boards[b], -1, E_BIST_THRESHOLD, (bist_iterations - 1) * 8 * 0.95));
...
//      batch.push_back(batchtype().Write(boards[b], -1, E_HASHCONFIG, (1<<16) + 0 + (0 << 3)));   // reset the miner just in case the fast clock wedged us
   ...
   ReadWriteConfig(batch);
   printf("%d engines are phase2 %.1f%%\n", 100.0 * phase2_count / phase4_count);
...
bool dvfstype::DVFS(const float THS, int experiment, bool verbose)     // returns true if performance could not be achieved or some error happened
   ...
   while (true) {
      ...
      hitrate = MeasureHashRate(period * (almost_there ? 1.0 : 1.0), too_hot, avg_temp, max_temp, experiment, !not_optimized_yet);
