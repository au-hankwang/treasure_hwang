...
void Fpga()
   ...
   t.WriteConfig(E_HITCONFIG, 1 << 31, -1);
   t.WriteConfig(E_PIN, 1 << 30, -1);
   t.IsAlive(0x3ff);
   t.SetBaud(1000000);
   t.IsAlive(0x3ff);
   t.SetBaud(2083333);
   ...
   int error = 0;
   for (i = 0; i < 128; i++)
      error += t.ReadConfig(E_UNIQUE, 0)!= 0x61727541 ? 1:0;
   printf("read errors=%d Comerror=%x, rsperror=%x\n",error,t.ReadConfig(E_COMERROR, 0), t.ReadConfig(E_RSPERROR, 0));
   t.IsAlive(0);
   t.ReadWriteStress(0,100);
   t.IsAlive(0);
   for (i = 0; i < 128; i++)
      error += t.ReadConfig(E_UNIQUE, 0) != 0x61727541 ? 1 : 0;
   printf("read errors=%d Comerror=%x, rsperror=%x\n", error, t.ReadConfig(E_COMERROR, 0), t.ReadConfig(E_RSPERROR, 0));
