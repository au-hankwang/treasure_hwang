...
void usbtype::BoardInfo(boardinfotype &info, bool quiet)
   ...
   printf("Implied Reference\t%6.3f V\n", info.implied_reference);
   ...
   for (int i = 0; i < 3 && false; i++) {
      printf("Raw[%d] %8x, %7.4f", i, info.raw_adc[i], info.raw_adc[i] / 65535.0 * info.implied_reference);
      printf("\tZero[%d] %8x, %7.4f\n", i, info.zero_offset[i], info.zero_offset[i] / 65535.0 / 256.0 * info.implied_reference);
      }
//   info.boards_present = 0x2;
   }
