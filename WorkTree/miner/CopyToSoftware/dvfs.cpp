...
void dvfstype::InitialSetup()  // this will setup the PLL's 
   ...
   for (i = 0; i < num_boards; i++) {
      ...
      batch.push_back(batchtype(i, -1, E_PLLCONFIG, 0x1d + ((pll_divider - 1) << 13))); // vcosel=0, div1=1, div2=2 (nominal vcoclk 1.6-3.2Ghz, hashclk is 800Mhz-1.6Ghz, assume vco can go lower than spec
      batch.push_back(batchtype(i, -1, E_PLLFREQ, systeminfo.min_frequency * pll_multiplier));

      if (systeminfo.refclk == 25.0)
         batch.push_back(batchtype(i, -1, E_IPCFG, 0x1)); // ipclk is 6.25Mhz
      else if (systeminfo.refclk == 40.0)
         batch.push_back(batchtype(i, -1, E_IPCFG, 0x2)); // ipclk is 5.0Mhz
      else if (systeminfo.refclk == 50.0)
         batch.push_back(batchtype(i, -1, E_IPCFG, 0x2)); // ipclk is 6.25Mhz
      else if (systeminfo.refclk == 100.0)
         batch.push_back(batchtype(i, -1, E_IPCFG, 0x3)); // ipclk is 6.25Mhz
      batch.push_back(batchtype(i, -1, E_TEMPCFG, 0xd));  // turn on temp sensor
      batch.push_back(batchtype(i, -1, E_SPEED_DELAY, 512));    // wait 512 cycles after changing pll before starting bist
      batch.push_back(batchtype(i, -1, E_SPEED_INCREMENT, 1.0 * pll_divider / systeminfo.refclk * (1 << 20)));    // makde speed search happen in 1Mhz increments
      batch.push_back(batchtype(i, -1, E_BIST_THRESHOLD, systeminfo.allowable_bad_engines));    
      }

   for (i = 0; i < num_boards; i++) {
      // i want a little delay between turning on the temp sensor and clearing max_temp
      batch.push_back(batchtype(i, -1, E_MAX_TEMP_SEEN, 0x0));  // clear max temp seen
      }
   for (i = 0; i < topology.size(); i++) {
      const topologytype& t = topology[i];
      batch.push_back(batchtype(t.board, t.id, E_THERMALTRIP, t.InverseTemp(systeminfo.thermal_trip_temp)));  // clear max temp seen
      }
...
void dvfstype::ProbeSubset(const vector<int>& subset, vector<int>& startf, vector<int>& maxf, vector<float>& temp)  // will return actual or theoretical THS
   ...
   for (i = 0; i < subset.size(); i++)
      ...
      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ)); // read out the current frequency
      batch.push_back(batchtype(t.board, t.id, E_TEMPERATURE)); // read out the current temp
      }
   ...
   for (i = 0; i < num_boards; i++)
      batch.push_back(batchtype(i, -1, E_SPEED_INCREMENT, increment));
   for (i = 0; i < subset.size(); i++) {
      int index = subset[i];
      const topologytype& t = topology[index];

      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, startf[index] * 0.98));
      batch.push_back(batchtype(t.board, t.id, E_SPEED_UPPER_BOUND, startf[index] * 1.1));
      batch.push_back(batchtype(t.board, t.id, E_BIST, 2)); // run bist
      }
   ...
   for (i = 0; i < subset.size(); i++)
      ...
      int index = subset[i];
      const topologytype& t = topology[index];
      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ)); // read the bist fail frequency
      }
   ReadWriteConfig(batch);
   ...
   for (i = 0; i < subset.size(); i++) {
      ...
      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, startf[index]));
      }
   ReadWriteConfig(batch);
   batch.clear();
   }
...
float dvfstype::AdjustAll(float max_tp, float &system_maxtemp, float& total_overtemp, int col)
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      if (col < 0 || t.col == col)
         batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, MAXIMUM(systeminfo.min_frequency * pll_multiplier, nextf[i])));
      if (false &&t.row == 15) {
...
float dvfstype::Optimize(float max_THS, bool &too_hot) // this function takes maximum TH/s and returns either the actual TH/s or the higher theoretical amount
   ...
   too_hot = total_overtemp > 10.0;
   if (actual_tp >= max_tp)
      printf("VDD:%.3fV Optimization done(%-3d iterations). %.1f TH/s requested and achieved. %.1f TH/s is possible at this operating point. MaxTj=%.1fC, limit=%.1fC %s\n", voltage, itcount, max_THS, actual_THS,maxtemp,systeminfo.max_junction_temp, too_hot ? "*Too Hot!" : "");
   else
      printf("VDD:%.3fV Optimization done(%-3d iterations). %.1f TH/s requested but only %.1f TH/s was possible at this operating point. MaxTj=%.1fC, limit=%.1fC %s\n", voltage, itcount, max_THS, actual_THS, maxtemp, systeminfo.max_junction_temp, too_hot?"*Too Hot!":"");
...
bool dvfstype::DVFS(float THS)     // returns true if performance could not be achieved or some error happened
   ...
   while (true) {
      ...
      else if (actual_ths < THS * 0.9) {   // we're way off so make a big jump
...
void dvfstype::SimpleOptimize()
   ...
   for (its = initial ? 0 : 20; its < 50; its++) {
      ...
      for (i = 0; i < topology.size(); i++) {
         ...
         if (temp[i] > systeminfo.max_junction_temp)
            batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, frequency[i] * 0.97));
         else if (temp[i] + 1.0 > systeminfo.max_junction_temp) {
            batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, frequency[i] * 0.97));
            batch.push_back(batchtype(t.board, t.id, E_SPEED_UPPER_BOUND, frequency[i] * 1.0));
            batch.push_back(batchtype(t.board, t.id, E_BIST, 2)); // run bist
            }
         else {
            batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, frequency[i] * 0.97));
            batch.push_back(batchtype(t.board, t.id, E_SPEED_UPPER_BOUND, frequency[i] * bounds));
            batch.push_back(batchtype(t.board, t.id, E_BIST, 2)); // run bist
            }
         }
      ReadWriteConfig(batch);
   ...
   for (i = 0; i < topology.size(); i++) {
      topologytype& t = topology[i];
      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ, frequency[i] - 2 * increment));
      }
   ReadWriteConfig(batch);
   batch.clear();
   initial = false;
...
void dvfstype::PrintSystem()
   ...
   for (i = 0; i < topology.size(); i++)
      ...
      batch.push_back(batchtype(t.board, t.id, E_PLLFREQ)); // read out the current frequency
      batch.push_back(batchtype(t.board, t.id, E_TEMPERATURE)); // read out the current temp
      batch.push_back(batchtype(t.board, t.id, E_VOLTAGE)); // read out the current temp
      }
