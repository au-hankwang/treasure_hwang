...
void testtype::Pll(float f_in_mhz, int id, bool pll1, int refdiv) {
   TAtype ta("Pll()");
   static bool reset_needed = true;
   float vco_center = 0;
   int best1 = 1, best2 = 1;
   int predivider = refdiv<=0 ? 5 : refdiv;
   float refclk = 25.0;
   float vco = (best1 + 1) * (best2 + 1) * f_in_mhz;
   pll_multiplier = (best1 + 1) * (best2 + 1) / (refclk / predivider) * (1 << 20);
   int fb_divider = (f_in_mhz / (refclk / predivider)) * (best1 + 1) * (best2 + 1) + 0.5;

   if (pll1) {
      if (reset_needed) {
         WriteConfig(E_PLL2, 0x12, id);   // bypass pll
         Delay(10);
         reset_needed = false;
         }
      if (f_in_mhz == 0) {
         WriteConfig(E_PLL2, 0, id);   // disable pll with no bypass
         reset_needed = true;
         }
      else if (f_in_mhz == 25) {
         WriteConfig(E_PLL2, 0x12, id);   // bypass pll
         ...
         WriteConfig(E_PLL2, 0x11 + (predivider << 12) + (best2 << 9) + (best1 << 6) + (fb_divider<<20), id);
      }
   else {
      ...
      if (f_in_mhz == 0) {
         ...
         WriteConfig(E_PLLCONFIG, 0x11 + (predivider << 20) + (best2 << 13) + (best1 << 10), id);

      WriteConfig(E_PLLFREQ, fb_divider << 20, id);
      }
//   printf("Setting pll for div1=%d div2=%d fb=%d, expected=%.1fMhz\n", best1, best2, fb_divider, fb_divider * refclk / (best1 + 1) / (best2 + 1));
   }
...
bool testtype::ReadWriteStress(int id, int iterations) {
   ...
   for (i = 0; i < regs.size(); i++) {
      expected[i] = 0;
      batch.push_back(batchtype().Write(current_board, id, regs[i], 0));
      }
...
bool testtype::FrequencyEstimate(float expected_hashclk) {  // return true on error
   ...
   int i;
   bool error = false;
   printf("\n --TEST :  Calling FrequencyEstimate()  \n");
   if (found.size() == 0) found.push_back(-1);
   printf("\n --TEST :  Calling FrequencyEstimate()  Found.size= %d  \n" , found.size());
...
void testtype::RegisterDump(const char* filename)
   {
   vector<batchtype> batch;
   int i, k;
   FILE* fptr;
   ...
   for (i = 0; i < topology.size(); i++) {
      ...
      batch.clear();
      for (k = 0; k < 256; k++)
         batch.push_back(batchtype().Read(t.board, t.id, k));
      ReadWriteConfig(batch);
      for (k = 0; k < 256; k++)
         fprintf(fptr, "%d:%d Reg%3d = 0x%-08X %d\n", t.board, t.id, k, batch[k].data, batch[k].data);
      }
   }
...
//      if (i % 100 == 0) printf("Testing %d, difficulty=%d, context=%d\n", i, difficulty,context);
      ...
      int tries;
      for (tries = 200; tries >= 0 && hits[i]<found.size(); tries--) {
         Delay(1);
         ReadWriteConfig(batch);
         for (k = 0; k < found.size(); k++) {
            if (batch[k].data & 0x7) {
               if (!GetHit(rsppacket, 0, found[k])) {
                  printf("hit error\n");
                  }
               else {
