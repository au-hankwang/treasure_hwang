...
#define SSD1306_WRITEDATA_MULTI(ptr, len)  ssd1306_I2C_Write_Multi(SSD1306_I2C_ADDR, 0x40, (ptr), (len))
/* Absolute value */
...
uint8_t SSD1306_Init(void) {
   HAL_Delay(100);	 // give some time for i2c to stabilize
...
//	SSD1306_WRITECOMMAND(0x12);
	SSD1306_WRITECOMMAND(0x0);	 // use this setting for 32x128 display
...
void OLED_Display(char* string1, char* string2, char* string3)
   ...
   if (string2[0] == 0 && string3[0] == 0 && strlen(string1) <= 8) {
      SSD1306_GotoXY(0, 32);
      SSD1306_Puts(string1, &Font_16x26, SSD1306_COLOR_WHITE);
      return;
      }
   else if (strlen(string2)<=12 && string3[0] == 0 && strlen(string1) <= 12) {
      SSD1306_GotoXY(0, 32);
      SSD1306_Puts(string1, &Font_11x18, SSD1306_COLOR_WHITE);
      SSD1306_GotoXY(0, 48);
      SSD1306_Puts(string2, &Font_11x18, SSD1306_COLOR_WHITE);
      return;
      }


   SSD1306_GotoXY(0, 32);
   SSD1306_Puts(string1, &Font_7x10, SSD1306_COLOR_WHITE);
   SSD1306_GotoXY(0, 43);
   SSD1306_Puts(string2, &Font_7x10, SSD1306_COLOR_WHITE);
   SSD1306_GotoXY(0, 54);
   SSD1306_Puts(string3, &Font_7x10, SSD1306_COLOR_WHITE);
//   SSD1306_UpdateScreen();
   }


// this needs to be called 8 times, 0-7
void SSD1306_UpdatePartialScreen(int row) {
   ...
   SSD1306_WRITEDATA_MULTI(&SSD1306_Buffer[SSD1306_WIDTH * row], 128);
//   for (int i = 0; i < 128; i++)
//      SSD1306_WRITEDATA(SSD1306_Buffer[SSD1306_WIDTH * row + i]);
...
void SSD1306_UpdateScreen() {
	...
	for (m = 0; m < 8; m++) {
		...
		for (int i=0; i<128; i++)
		   SSD1306_WRITEDATA(SSD1306_Buffer[SSD1306_WIDTH * m + i]);
		/* Write multi data */
