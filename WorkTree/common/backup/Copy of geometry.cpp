...
// this algorithm is not optimal, but plenty fast and I didn't have to resort to balanced binary trees
// outputs should be empty on entry
void polytype::Merge(array<polytype *> &outputs, const array<const polytype *> &inputs, int operation)  // 0=union, 1=intersection
   ...
   while (tops.size() || bottoms.size())
	  ...
	  pointtype p;(tops[topindex].low, tops[topindex].extra);
	  ...
	  while (true)  // this will walk around positive polygons
		 ...
		 for (i=0; i<tops.size(); i++)
			{
			y = tops[i].extra;
			if (tops[i].low == p.x && tops[i].node==0)
			   {
		 ...
		 for (i=0; i<bottoms.size(); i++)
			{
			y = bottoms[i].extra;
			if (bottoms[i].low == p.x && bottoms[i].node==0)
			   {
...
//		 if (tabovey < babovey && taboveindex>=0 && tbelowy < bbelowy && tbelowindex>=0)
//			FATAL_ERROR;
//		 if (babovey < tabovey && baboveindex>=0 && bbelowy < tbelowy && bbelowindex>=0)
//			FATAL_ERROR;

		 intervaltype *ii = NULL;
		 if (walkpositive)
			{
			if (lasttop && tabovey < babovey && taboveindex>=0)
			   {
			   ii = &tops[taboveindex];
			   lasttop = true;
			   }
			else if (lasttop && tbelowy < bbelowy && tbelowindex>=0)
			   {
			   ii = &tops[tbelowindex];
			   lasttop = true;
			   }
			else if (lasttop && babovey < tabovey && baboveindex>=0)
			   {
			   ii = &bottoms[baboveindex];
			   lasttop = false;
			   }
			else if (lasttop && bbelowy < tbelowy && bbelowindex>=0)
			   {
			...
			else if (!lasttop && tabovey < babovey && taboveindex>=0)
			   {
			   ii = &tops[taboveindex];
			   lasttop = true;
			   }
			...
			{
			if (!lasttop && tbelowy < bbelowy && tbelowindex>=0)
			   {
			   ii = &tops[tbelowindex];
			   lasttop = true;
			   }
		 ...
		 printf(" y=%5d %s[%d]\n",ii->extra,lasttop?"tops":"bots",lasttop? (ii-&tops[0]) : (ii-&bottoms[0]));
   ...
   for (i=0; i<negatives.size(); i++)
	  ...
	  for (k=0; k<poly.points.size(); k++)
		 ...
		 if (poly.points[k].y == p.y)
			{
			int d = labs(p.x - poly.points[k].x);
...
void DetectCollisionsAndMerge(array<polytype *> &out, array<polytype *> &in)
   ...
   for (i=0; i<in.size(); i++)
      ...
      if (connect.Handle(i) == i)
         ...
         polytype::Merge(tempoutput, groups, 0);
...
// I also want to give preferrence to small ears first. (ie ie want to carve off long narrow piece before short fat ones)
   ...
   while (points.size()>4 && count<10000)
      {
      count++;
      bool found=false;
