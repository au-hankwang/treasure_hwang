...
public:
   ...
   pointtype Transform(pointtype l) const
      {
      pointtype g;
      g.x = (int)(0.5+matrix[0][0] * l.x + matrix[0][1] * l.y + matrix[0][2]);
      g.y = (int)(0.5+matrix[1][0] * l.x + matrix[1][1] * l.y + matrix[1][2]);
      return g;
      }
   ...
   recttype Transform(recttype l) const
      ...
      p2.x = (int)(0.5+matrix[0][0] * p0.x + matrix[0][1] * p0.y + matrix[0][2]);
      p2.y = (int)(0.5+matrix[1][0] * p0.x + matrix[1][1] * p0.y + matrix[1][2]);
      p3.x = (int)(0.5+matrix[0][0] * p1.x + matrix[0][1] * p1.y + matrix[0][2]);
      p3.y = (int)(0.5+matrix[1][0] * p1.x + matrix[1][1] * p1.y + matrix[1][2]);
   ...
   pointtype Inverse(pointtype g) const
      ...
      l.x = (int)(0.5+( matrix[1][1]*g.x - matrix[0][1]*g.y + matrix[1][2]*matrix[0][1] - matrix[0][2]*matrix[1][1]) / z);
      l.y = (int)(0.5+(-matrix[1][0]*g.x + matrix[0][0]*g.y + matrix[0][2]*matrix[1][0] - matrix[1][2]*matrix[0][0]) / z);
...
public:
   ...
   boxtype(enum layertype _layer, const recttype &_r)
      {
      clear();
      layer = _layer;
      r = _r;
...
public:
   ...
   int handle;  // erase me, I temporarily added this for armadillo
   int node;
   ...
   polytype(enum layertype _layer, const recttype &r)
      {
      clear();
      layer = _layer;
...
//   void Paint(wxDC &dc, const translationtype &t, const PENtype &pen, bool ignoreComment=false) const;
   void OutToDLF(FILE *fptr, mbbtype &mbb, const int internalScale, const int units);
   void BoxConvert(array<boxtype> &boxes, float max_aspect_ratio);
