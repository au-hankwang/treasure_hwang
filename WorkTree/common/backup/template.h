...
*/   void pop_back()
   ...
   void Insert(int before, const array &right)
      {
      int i;
      if (before <0 || before >top) FATAL_ERROR;
      ...
      for (i=0; i<right.top; i++)
         {
         if (i + before >= top)
            CopyConstruct(i+before, right.arr[i]);
         else
            arr[i + before] = right.arr[i];
         }
      top += right.top;
      }
   ...
   void RemoveFast(int index)  // performs an erase in O(1) time but changes order of elements >= index
      {
      if (index >=top) FATAL_ERROR;
      arr[index] = arr[top-1];
      pop_back();
      }   
   void Remove(int index)  // performs an erase in O(N) time but it doesn't reorder
      {
      if (index >=top) FATAL_ERROR;
...
// simpler and less flexible than STL template but it memory accesses in block
// and doesn't depend on linked lists
template <class T> class queuetype
   {                                                                                        
   int top,bottom,allocated;
   T *queue;
   
public:         
   queuetype(int initial_size=1024)
      {
      initial_size = MAXIMUM(initial_size, 16);
      queue=NULL;
      bottom=top=0;
      allocated=initial_size;
      queue=new T[allocated];
      if (queue==NULL) FATAL_ERROR;
      }
   ~queuetype() 
      {
      if (queue!=NULL) 
   ...
   //copy constructor
   queuetype (const queuetype &original)
      {
      allocated = original.allocated;
      top = original.top;
      bottom = original.bottom;
      arr= new T[allocated];
      if (arr==NULL) FATAL_ERROR;  // out of memory most likely
   ...
   // assignment operator
   queuetype operator=(const queuetype &right)
      {
      if (this == &right)
         return *this;

      if (top!=bottom) // you are overwriting and existing queue. May sure the stuff you're overwriting doesn't require destruction
         {
         FATAL_ERROR;
         }
   ...
   int size()
      ...
      else return (allocated-bottom + top);
      }
   ...
   void push(T &ob)
      ...
      if (top==bottom)
         { // must grow queue
         queuetype newqueue(allocated*2);
         T temp;
         
         while (pop(temp))
            newqueue.push(temp);
         
         // now clean up old queue
         delete []queue;
         top=newqueue.top;
         bottom=newqueue.bottom;
         allocated=newqueue.allocated;
         queue=newqueue.queue;
         newqueue.queue=NULL;  // this prevents destructor from doing any damage
         }    
      }
...
public:
   ...
   void StartDump(int &internal) const 
      {
      internal = 0;
      }
   bool DumpKeys(int &internal, KEY &key, PAYLOAD &payload) const // returns false if no more keys not seen
      {      
      for (internal; internal<allocated; internal++)
         if (IsPresent(internal))
            {
            key     = table[internal].key;
            payload = table[internal].payload;
            internal++;
            return true;
            }
      return false;
...
inline void sha(const void *mem, int memsize, __uint64 &md1, __uint64 &md2)
   {
//   TAtype ta("SHA-1()");
   unsigned int w[16];
...
//            FATAL_ERROR;
      return md1==right.md1 && md2==right.md2;
      }
...
public:
   ...
   bool DumpKeys(int &internal, const char *&txt, PAYLOAD &payload)  // returns false if no more keys not seen      
      {
      texthashkeytype tk;
      bool ret = hashtype<texthashkeytype, PAYLOAD>::DumpKeys(internal, tk, payload);
      txt = tk.txt;
      return ret;
      }
...
public:
   ...
   void resize(int _max_objects)
      {
      for (int i=table.size(); i<_max_objects; i++)
         table.push(i);
      }
...
private:
#ifdef __alpha
   #define nativeSize 64
   #define words  ((N+63)>>6)      // a const int requires storage!
   #define native __uint64
#else
   #define nativeSize 32
   #define words  ((N+31)>>5)      // a const int requires storage!
   #define native unsigned 
#endif
...
public:
   ...
   bitfieldtype(const native right)
      {
      operator=(right);
      }
   bitfieldtype &operator=(const native right)
      {
      data[0] = right;
...
*/
   ...
   bitfieldtype operator+(const bitfieldtype &right) const
      ...
      for (int i=0; i<words; i++)
         ...
         const native cin = (data[i] ^ right.data[i] ^ ret.data[i]);
         carry = ((cin & data[i]) | (cin & right.data[i]) | (data[i] & right.data[i])) >>(nativeSize-1);
         }
      return ret;
      }
   bitfieldtype operator-(const bitfieldtype &right) const
   ...
   bitfieldtype operator<<(int left) const
      ...
      native carry=0;
      bitfieldtype ret;
      if (left%nativeSize == 0)
         {
         for (int i=words - left/nativeSize-1; i>=0; i--)
            {
            ret.data[left/nativeSize + i] = data[i];
            }
         return ret;
         }
      for (int i=0; i<words - left/nativeSize; i++)
         {
         native nextcarry = data[i] >> (nativeSize - (left%nativeSize));
         ret.data[left/nativeSize + i] = (data[i]<<(left%nativeSize)) | carry;
         carry = nextcarry;
         }
      return ret;
      }
   bitfieldtype operator>>(int right) const
      ...
      native carry=0;
      bitfieldtype ret;
      for (int i=words - right/nativeSize-1; i>=0; i--)
         {
         native nextcarry = data[right/nativeSize + i] << (nativeSize - (right%nativeSize));
         ret.data[i] = (data[right/nativeSize + i]>>(right%nativeSize)) | carry;
         carry = nextcarry;
         }
      return ret;
      }
   ...
   bool GetBit(const int bit) const
      {
      if (bit <0 || bit >= N) FATAL_ERROR;
      return 0 != (data[bit>>6] &  ((native)1<<(bit&63)));
      }
   operator __uint64() const
      {
      return data[0];
      }   
   char *Print(char *buffer) const
      {
      char *chptr=buffer;
      int i;
      
      for (i=words-1; i>0 && data[i]==0; i--)   // skip over leading zeros
         ;
   ...
   bool GetBit(const int bit) const
      {
      if (bit <0 || bit >= N) FATAL_ERROR;
      return 0 != (data[bit>>5] &  ((native)1<<(bit&31)));
      }
/*   operator __uint64() const
      {
      if (words>=2)
         return data[0] | ((__uint64)data[1]<<32);
      else
         return data[0];
      }*/
   char *Print(char *buffer) const
      ...
      for (i=words-1; i>=1; i--)   // skip over leading zeros
         if (data[i]!=0)
            break;
   ...
   int DestructivePopCount()  // will destroy data but be fast (32 bit version)
      {
      native a, b;
      int i, count;
      for (i=0; i<words; i++)
