...
public:
   ...
   arraytype(void *_external_mem=NULL, size_t external_size=0) : external_mem(_external_mem)
      {
      top = 0;
      if (external_mem)
         {
   ...
   arraytype(size_t n, const T &val = T(), void *_external_mem=NULL, size_t external_size=0) : external_mem(_external_mem)
      {
      top = 0;
      if (external_mem)
         {
   ...
   arraytype(const arraytype &right) : external_mem(NULL)  // copy constructor
      {
      arr = NULL;
      top = 0;
      allocated = 0;
   ...
   // push is tricky because val could be an element in this arraytype, after a relocation it would be invalid
...
*/   void pop_back()
   ...
   // this is necessary because the compiler will change fooarraytype=5 into fooarraytype = arraytype<footype>(5)
   void operator=(const int);  // will result in linker error! Fix the code.
   arraytype &operator=(const arraytype &right)
      {
      size_t i;
      if (this == &right)
         return *this;
   ...
   bool operator==(const arraytype &right)
      {
   ...
   void Insert(int before, const arraytype &right)
      {
...
/*
   ...
   int BinarySearchIndex(const T &value, size_t min=0, size_t max=0) const // will return -1 if the item isn't in the sorted list, min/max are sub ranges in arraytype inclusive
      {
      const T *ptr;
      if (max==0 && min==0)
         max = top-1;
   ...
   void RemoveDuplicates()   // arraytype will be unordered after this
      {
      Sort();
   ...
   // this function takes two sorted arraytypes and returns an arraytype of all the common elements. O(N) complexity
   void CommonContents(arraytype <T> &result, const arraytype <T> &right) const
      {
      int l, r;
   ...
   // this function takes two sorted arraytypes deletes occurence of right in this. O(N) complexity
   void RemoveCommonElements(const arraytype <T> &right)
      {
      int l, r;
      arraytype <int> deleted;
   ...
   // this function takes two sorted arraytypes and returns an arraytype of all the elements in right not it self. O(N) complexity
   void AdditionalContents(arraytype <T> &result, const arraytype <T> &right) const
      {
      int l, r;
...
template <class KEY, class PAYLOAD> class hashtype
   ...
   unsigned allocated;
   int *bitfield;    // packed bit arraytype telling whether entry is used or not
...
private:
   arraytype<const char *> stringPools;
   char *pool;
   int poolsize;
...
class wildcardtype{
   ...
   mutable arraytype <char> allPatterns;   // will be like (pat1)|(pat2)...
