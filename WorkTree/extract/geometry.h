...
public:
   ...
   void FetchHorizontalInsideIntervals(int y, arraytype<intervaltype> &intervals) const;
...
//   void Paint(wxDC &dc, const translationtype &t, const PENtype &pen, bool ignoreComment=false) const;
   void OutToDLF(FILE *fptr, mbbtype &mbb, const int internalScale, const int units);
   void BoxConvert(arraytype<boxtype> &boxes, float max_aspect_ratio, float max_step_ratio);
   void BoxConvert2(arraytype<boxtype> &boxes) const;
   void BoxAdjust(arraytype<boxtype> &boxes, float max_aspect_ratio, float max_step_ratio);
   void Cleanup();
   ...
   static void Merge(arraytype<polytype *> &output, const arraytype<const polytype *> &input, int operation);  // 0=union, 1=intersection, 2=xor
   static void OldMerge(arraytype<polytype *> &output, const arraytype<const polytype *> &input, int operation);  // 0=union, 1=intersection, 2=xor
   static void Subtract(arraytype<polytype *> &output, const arraytype<const polytype *> &input);  // input[0] will be subtracted by inputs[1..n]
   static void SmartMerge(arraytype<polytype *> &polygons);  // only merges polygons on same layer, input should be deletable
   static void Display(const char *title, arraytype<polytype *> polygons);  // Pops up a window showing polygons
   };
