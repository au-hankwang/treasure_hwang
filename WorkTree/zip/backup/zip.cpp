...
//      printf("\ncount=%d Code=%d h=%d",count,code,h);
      if(code == -1) {
         ref.error_out = 4;
   ...
   if(!ref.unzipdone || doit) {
      ...
      if(ref.unzipdone) { // error or found_eof
         ...
         if(ref.error_out==1) {
            ...
            unzip_flush_out(ref);
            if (reference!=NULL && ref.c.s.total_bytes_sent_out != reference->size()) 
               FATAL_ERROR;
            }
...
void unzip_byte_out(reftype &ref, unsigned char byte, const array <unsigned char> *reference) {
   ...
   if (reference!=NULL && byte != (*reference)[ref.c.s.total_bytes_out]) 
      FATAL_ERROR;
...
void BlockStuff(array <__uint64> &recodes, array <unsigned char> &temp, int &tempbit)
   {
   ...
   temp.push(0);
   temp.push(0);
   recodes.push(temp[0] | (temp[1]<<8) | (temp[2]<<16) | ((__int64)temp[3]<<24) | ((__int64)temp[4]<<32) | ((__int64)temp[5]<<40) | ((__int64)temp[6]<<48) | ((__int64)tempbit<<56));
   temp.clear();
   tempbit = 0;
   }


ZIP_BLOCK_TYPES OutputBlock(reftype &ref, array <unsigned char> &output, int &bit, int rawsize, array <matchtype> &matches, blocktype *blockp, bool final, bool force_fixed, bool force_dynamic, bool lzs, const unsigned char *allow_nocompression, int matchbundles, bool printit=true);
...
// this code is to collect raw output codes to compare against RTL
   if (lzs && block_present)
      ...
      if (throw_bits>0)
         block.recodes.push(0);  // padding bits
      }
...
void zip_byte_init(reftype &ref, int inputlen) {
   ...
   ref.searches = 0;
   ref.totalsearches = 0;
   ref.string_distribution.clear();
   ref.string_distribution.resize(8,0);
   ref.search_cycles = 0;
   ref.nblocks = 0;
   ref.matchbundles = 0;
   ref.lastlen = 0;
...
void zip_byte(reftype &ref, bool printit) {
   ...
   if(ref.input_so_far == ref.inputlen) {
      ...
      while (ref.lmatches->size()!=0){
         ...
         if (ref.matchbundles>(ref.allow_nocompression ? BLOCKSIZE/16 :BLOCKSIZE) && !ref.force_fixed) {
         ...
         ZIP_BLOCK_TYPES t = OutputBlock(ref.output, ref.bit, 1+ref.z_i-ref.startblock, *(ref.lmatches), tblockp, final, ref.force_fixed, ref.force_dynamic, ref.lzs, (ref.allow_nocompression && ref.startblock<ref.input.size()) ? &(ref.input[ref.startblock]) : NULL, ref.matchbundles, printit);
...
/*
   if (ref.storage && ref.z_i>=ref.skipto)
      {
      array <int> odd[2], even[2];
      array <int> possibles;
      int starti=ref.z_i, num=0, lazy=0, bestentry=-1, maxlen=0, dist=-1, l, k, i, location;

      while (ref.z_i + lazy<ref.inputlen && lazy<1) 
         {
         ref.zhash->Check(ref.input, ref.z_i+lazy, ref, false, odd[lazy], even[lazy]);
         ref.zhash->Add(  ref.input, ref.z_i+lazy, ref);

         if (odd[lazy].size()==0 && even[lazy].size()==0) 
            {
            lazy++;
            break;
            }
         if (odd [lazy].size()>2)  odd[lazy].resize(2);
         if (even[lazy].size()>2) even[lazy].resize(2);
         lazy++;
         }
      ref.completelyskip = ref.z_i + lazy;
//      if ( odd[0].size() +  odd[1].size()>2 &&  odd[0].size()>1)   odd[0].resize(1);
//      if (even[0].size() + even[1].size()>2 && even[0].size()>1)  even[0].resize(1);
//      if ( odd[0].size() +  odd[1].size()>2)  odd[1].resize(2- odd[0].size());
//      if (even[0].size() + even[1].size()>2) even[1].resize(2-even[0].size());

      for (i=0; i<lazy; i++)
         {
         for (k=0; k<odd[i].size(); k++)
            possibles.push(odd[i][k] | (i<<16));
         for (k=0; k<even[i].size(); k++)
            possibles.push(even[i][k] | (i<<16));
         }
      ...
      for (k=0; k<possibles.size(); k++) 
         ...
         if (i-location >= 4096) 
            l=0;          // storage mode only has 4K history
         if (ref.lzs && i-location >= 2048) 
      ...
      if (maxlen >= ref.string_distribution.size())
         ref.string_distribution.back()++;
      else
         ref.string_distribution[maxlen]++;
      }
   else */
   if (ref.z_i<ref.completelyskip)
      ...
      while (ref.z_i + lazy<ref.inputlen && lazy<(ref.compression_fastest?1:ref.lzs?2:3)) 
         {
         array <int> odd, even;
      ...
      for (k=0; k<possibles.size(); k++) {
         ...
         if (i-location >= 4096 && ref.storage)
            l=0;          
      ...
      if (bestentry<0 || maxlen<=2 || (maxlen<4 && !(ref.force_fixed || ref.lzs))) 
         {
         for (k=0; k<lazy; k++) {
            ref.lmatches->push(matchtype(ref.input[k+starti]));
            ref.matchbundles++;
