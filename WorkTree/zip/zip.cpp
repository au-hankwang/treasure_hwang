   ...
   if (ref.lzs)
      ...
      if(code == -1) {
         ...
         if (t==0) // EOB
            ...
            //	    printf("total_bits=%x throw=%d\n",ref.total_bits_read,throw_bits);
            if (throw_bits!=0){
         ...
         if(ref.lzs && t == 0) {
            ref.error_out = 11;
            ref.error_position = 200020; // stupid unique identifier to find where error happened in the code
            ref.unzipdone = true;
            }
         int distance = t, length=0;
         if(h < distance) {
...
void unzip(reftype &ref, const array <unsigned char> *reference) {
   ...
   ref.input_byte_xref.clear();
   ref.input_byte_xref.resize(ref.output.size(), -1);
   unzip_byte_init(ref, inputsize, true/*bof*/, true/*eof*/, true/*nodynamicstop*/);
   if (inputsize == 0)
      { 
      ref.error_out = 4;
      ref.error_position = 103459; // stupid unique identifier to find where error happened in the code
      return;
      }
   do {
      unzip_byte(ref, reference);
      } while(!ref.unzipdone);
   if (ref.input_byte_xref.size() != ref.output.size())
      {
      printf("input_byte_xref.size()=%d != ref.output.size()=%d\n",ref.input_byte_xref.size(), ref.output.size());
//      FATAL_ERROR;
      }
   }
...
void unzip_byte_out(reftype &ref, unsigned char byte, const array <unsigned char> *reference) {
   ...
   ref.input_byte_xref.push((ref.bit-1)/8);
...
ZIP_BLOCK_TYPES OutputBlock(reftype &ref, array <unsigned char> &output, int &bit, int rawsize, array <matchtype> &matches, blocktype *blockp, bool final, bool force_fixed, bool force_dynamic, bool lzs, const unsigned char *allow_nocompression, int matchbundles, bool printit) {
   ...
   if (lzs) litHuff[0].occurence++;  // lzs with history can result in no literal. This breaks the lithuff.computelength function even though it's result doesn't matter
   ...
   if (allow_nocompression && !ref.lzs &&((dynamiccost%7)%3)==1 && rawsize<=65535)
      {
      //      printf("Rawsize=0x%X\n",rawsize);
      SetBits(output, bit, 1, final && ref.eof);
      ...
      if (!ref.lzs)
         {
         SetBits(output, bit, 1, final && !ref.syncflush && ref.eof);
         ...
         if(block_present) {
            block.alphalens.push(((final && ref.eof && !ref.syncflush) ? 0xfded : 0xfdec) | (16<<16));
      ...
      if (lzs) 
         ...
         SetBits(output, bit, 1, final && ref.eof && !ref.syncflush);
         SetBits(output, bit, 2, 1);  // fixed huffman
         if(block_present) {
            block.alphalens.clear();
            block.alphalens.push(((final && ref.eof && !ref.syncflush) ? 0x3 : 0x2) | (3<<16));
            }
   ...
   if (lzs)
      ...
      if (ref.syncflush && final)  // add a no compress block to force byte alignment
         {
         SetBits(output, bit, 1, ref.eof);
         SetBits(output, bit, 2, 0);  // no compress block
         int nocompress_padding = (-bit)&7;
         SetBits(output, bit, nocompress_padding, 0);   // pad up to nearest byte
         SetBits(output, bit, 8, 0x00);
         SetBits(output, bit, 8, 0x00);
         SetBits(output, bit, 8, 0xff);
         SetBits(output, bit, 8, 0xff);
         
         block.alphalens.push((ref.eof ? 0x1 : 0x0)    | (3<<16));
         if (nocompress_padding)
            block.alphalens.push(0x0    | (nocompress_padding<<16));
         block.alphalens.push(0x0    | (16<<16));
         block.alphalens.push(0xffff | (16<<16));
         }
   ...
   if (lzs && block_present)
      ...
      if (ref.syncflush && final)  // add a no compress block to force byte alignment
         {
         SetBits(temp, tempbit, 1, ref.eof);
         SetBits(temp, tempbit, 2, 0);  // dynamic huffman
         for (i=0; (tempbit&7); i++)
            SetBits(temp, tempbit, 1, 0);   // pad up to nearest byte
         SetBits(temp, tempbit, 8, 0x00);
         SetBits(temp, tempbit, 8, 0x00);
         SetBits(temp, tempbit, 8, 0xff);
         SetBits(temp, tempbit, 8, 0xff);
         BlockStuff(block.recodes, temp, tempbit);
         }
      }
   ...
   if(ref.input_so_far == ref.inputlen) {
      ...
      while (ref.lmatches->size()!=0){
         ...
         *(ref.lmatches) = deferred;
         ...
         if(printit) printf("ZIP: Hash widget: %.2fb/cycle, History widget: %d (%.2f/Byte)total searches needed. %.2fb/cycle.\n",
...
void zip_byte_buffered(reftype &ref, bool printit) 
   ...
   const int minlen    =  ref.lzs ? 2 : ref.force_fixed ? 3 : 4;
   const int maxlength =  ref.lzs ? 0xff7 : 258;
   if (ref.z_i<ref.completelyskip)
      ...
      {
      array <int> possibles;
      int starti=ref.z_i, lazy=0, bestentry=-2, maxlen=0, dist=-1, l, k, location;
      bool bestbackup=false;
      array <int> odd, even;
      ...
      if (!ref.backup_hash){
         ...
         {
         odd = ref.zhash->last_odd;
         even = ref.zhash->last_even;
         }
      ...
      for (k=0; k<possibles.size(); k++) {
         ...
         bool backupone=i>0 && location>0 && (location&0xf) && ref.input[i-1]==ref.input[location-1];
         for (l=0; l<maxlength && location>=i-32768 && location<i && location>=0 && i+l<ref.inputlen && ref.input[i+l]==ref.input[location+l]; l++)
            ;
     ...
     if (ref.inputlen<4)
        l=0;
         bool better = false;
         better = better || (l>maxlen);
         better = better || (l>=maxlen && (i-location)<dist);

         if (better && l>=minlen) {
            maxlen    = l;
            dist      = i - location;
            bestentry = 0;
            bestbackup= backupone;
            }
         }
      lazy=1;
      ...
      possibles.clear();
      if (maxlen>=minlen)
         { // I found a match now I need to search the next four entries
         while (ref.z_i + lazy<ref.inputlen && lazy<3)
            {
            array <int> odd, even;
         ...
         for (k=0; k<possibles.size(); k++) {
            ...
            while (entry>=1 && location>0 && (location&0xf))
               {
      ...
      if (maxlen < lazy && maxlen>=minlen)
         {
         ref.backup_hash = 1; // I walked past the string so I need to backup the hash stuff
         lazy --;
         }
      else
         ref.backup_hash = 0;
      ...
      bestbackup = bestbackup && ref.lastliteral>=0 && bestentry==0 && maxlen>=minlen && !(maxlen&0x180);
      if (bestbackup)
         {
         if (ref.lastliteral<0) FATAL_ERROR;
         ref.lastliteral = -1;
         }
      if (ref.lastliteral>=0)
         {
         ref.lmatches->push(matchtype(ref.lastliteral));
         ref.matchbundles++;
         ref.lastliteral = -1;
         }
      if (maxlen<minlen)
         ...
         ref.lastliteral = ref.input[k+starti];
         ref.skipto = starti+k;
         }
      else {
         if (bestentry < -1)
         ...
         ref.lmatches->push(matchtype(maxlen + (bestbackup?1:0), dist));
         ref.matchbundles += (ref.matchbundles&1)+2;
         ref.skipto = starti+bestentry+maxlen;
         }
      }
   if (ref.z_i==ref.input.size()-1 && ref.lastliteral>=0)
      {
      ref.lmatches->push(matchtype(ref.lastliteral));
      ref.matchbundles++;
      ref.lastliteral = -1;
   ...
   if ((ref.matchbundles >= (ref.allow_nocompression ? BLOCKSIZE/16: BLOCKSIZE)) && !ref.force_fixed && (ref.z_i+1 < ref.inputlen) && (ref.skipto < ref.inputlen)) {
      ...
      int bit_before = ref.bit;
      bool final = false;
      ZIP_BLOCK_TYPES t = OutputBlock(ref, ref.output, ref.bit, 1+ref.z_i-ref.startblock, *(ref.lmatches), tblockp, final, ref.force_fixed, ref.force_dynamic, ref.lzs, ref.allow_nocompression ? &(ref.input[ref.startblock]) : NULL, ref.matchbundles, printit);
      ref.block_data[ref.nblocksp].output_bits = ref.bit - bit_before;
      ref.block_data[ref.nblocksp].type = t;
      ref.nblocksp ++;
#else
      ...
      blocktype *tblockp = &(ref.blocks.back());
      bool final = false;
      (void) OutputBlock(ref, ref.output, ref.bit, 1+ref.z_i-ref.startblock, *(ref.lmatches), tblockp, final, ref.force_fixed, ref.force_dynamic, ref.lzs, ref.allow_nocompression ? &(ref.input[ref.startblock]) : NULL, ref.matchbundles, printit);
#endif
      ref.lmatches->clear();
      *(ref.lmatches) = deferred;
      ref.matchbundles=0;
